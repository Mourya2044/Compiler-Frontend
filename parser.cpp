  ...
  while (changed) {
    ...
    for (auto& p : grammar) {
      for (int i = 0; i < (int)p.rhs.size(); i++) {
        ...
        if (trailEps) {
          for (auto& s : FOLLOW[p.lhs]) changed |= FOLLOW[B].insert(s).second;
        }
      }
    }
  }
  ...
  while (added) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (auto& item : states[i])
        if (item.dot < (int)item.rhs.size()) symbols.insert(item.rhs[item.dot]);
...
void buildParsingTable() {
  for (int i = 0; i < (int)states.size(); i++) {
    for (auto& item : states[i]) {
      if (item.dot < (int)item.rhs.size()) {
        ...
        for (int p = 0; p < (int)grammar.size(); p++) {
          if (grammar[p].lhs == item.lhs && grammar[p].rhs == item.rhs) {
            k = p;
            break;
          }
        }
    ...
    for (auto& A : nonterminals)
      if (transitions.count({i, A})) GOTO[{i, A}] = transitions[{i, A}];
  }
}
...
void printParsingTables() {
  ...
  const int stateW = 8;
  const int symbolW = 16;
  const int actionW = 18;

  auto line = [&](char c) {
    cout << "+" << string(tableWidth - 2, c) << "+\n";
  };

  auto centeredTitle = [&](const string& title) {
    int inner = tableWidth - 2;
    int padLeft = max(0, (inner - (int)title.size()) / 2);
    int padRight = max(0, inner - padLeft - (int)title.size());
    cout << "|" << string(padLeft, ' ') << title << string(padRight, ' ')
         << "|\n";
  };

  cout << "\n";
  line('=');
  centeredTitle("FIRST SETS");
  line('=');
  ...
  cout << "\n";
  line('=');
  centeredTitle("PARSING TABLE");
  line('=');

  cout << "\nACTION TABLE:\n";
  cout << "+" << string(stateW, '-') << "+" << string(symbolW, '-') << "+"
       << string(actionW, '-') << "+\n";
  cout << "|" << left << setw(stateW) << "State"
       << "|" << left << setw(symbolW) << "Terminal"
  ...
  for (auto it = ACTION.begin(); it != ACTION.end(); ++it) {
    cout << "|" << left << setw(stateW) << it->first.first << "|" << left
         << setw(symbolW) << it->first.second << "|" << left << setw(actionW)
         << it->second << "|\n";
  }
  cout << "+" << string(stateW, '-') << "+" << string(symbolW, '-') << "+"
       << string(actionW, '-') << "+\n";
  ...
  cout << "+" << string(stateW, '-') << "+" << string(symbolW, '-') << "+"
       << string(actionW, '-') << "+\n";
  cout << "|" << left << setw(stateW) << "State"
       << "|" << left << setw(symbolW) << "Nonterminal"
       << "|" << left << setw(actionW) << "NextState"
       << "|\n";
  cout << "+" << string(stateW, '-') << "+" << string(symbolW, '-') << "+"
       << string(actionW, '-') << "+\n";
